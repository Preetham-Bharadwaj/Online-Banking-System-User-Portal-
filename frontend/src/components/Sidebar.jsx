    ...
    <aside className="w-72 bg-white border-r border-slate-100 flex flex-col hidden lg:flex">
      <div className="h-28 flex items-center px-12">
        <Link to="/" className="flex items-center gap-3 text-primary-600 font-black text-2xl">
          ...
          <span className="tracking-tighter">Finova</span>
        </Link>
      </div>
        ...
        {navigation.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              to={item.href}
              className={`flex items-center gap-4 px-6 py-4 rounded-[1.5rem] text-[13px] font-black transition-all group ${isActive
